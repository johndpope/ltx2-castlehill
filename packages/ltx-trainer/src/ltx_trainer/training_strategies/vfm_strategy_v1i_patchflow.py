...
class VFMv1iPatchFlowConfig(VFMv1cTrainingConfig):
...
class VFMv1iPatchFlowStrategy(VFMv1cTrainingStrategy):
    ...
    def _prepare_distill_inputs(
        ...
        # For simplicity, reuse v1d distill logic but add uncertainty head
        # (Full implementation would mirror _prepare_patchflow_inputs + teacher targets)
        cfg = self.config
        # ... (similar to v1d _prepare_distill_inputs, omitted for brevity in this sketch)
        # Key: still predict per_token_logvar and include NLL loss in compute_loss
        raise NotImplementedError("Distill + Patch Forcing path not fully implemented — use distill_mode='none' for now")
    ...
    def compute_loss(
        ...
        if per_token_logvar is not None and cfg.uncertainty_weight > 0:
            from patch_flow.diagonal_gaussian import DiagonalGaussian  # from provided code

            sigma_theta = torch.exp(0.5 * per_token_logvar)
            # Use velocity prediction as mean (standard in flow matching)
            pred_theta = DiagonalGaussian(mean=video_pred.detach(), std=sigma_theta)
            ut = inputs.video_targets  # z - x₀

            sigma_loss = pred_theta.nll(ut)
            if inputs.video_loss_mask is not None:
                mask = inputs.video_loss_mask.unsqueeze(-1).float()
                sigma_loss = (sigma_loss * mask).sum() / mask.sum().clamp(min=1) / sigma_loss.shape[-1]
            else:
            ...
            if active_logvar.numel() > 0:
                self._last_vfm_metrics.update({
                    ...
                    "vfm/sigma_theta_mean": sigma_theta[per_token_logvar != 0].mean().item(),
                })
